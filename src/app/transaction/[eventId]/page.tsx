...
//         return decodedPayload.id || null; // your JWT has 'id'
//       } catch (err) {
...
//                 await apiCall.post("/transaction", payload, {
//                   headers: {
                  ...
                  {values.tickets.map((ticket, index) => (
                    ...
                    >
                      <div className="flex-1 flex flex-col gap-1 w-full">
                        ...
                        <Select
                          onValueChange={(val) =>
                            setFieldValue(`tickets[${index}].ticket_id`, Number(val))
                          }
                          value={
                            values.tickets[index].ticket_id
                              ? String(values.tickets[index].ticket_id)
                        ...
                        >
                          ...
                          <SelectContent>
                            {eventTickets.map((t) => {
                              const isSelectedElsewhere = values.tickets.some(
                                (ticket, i) =>
                                  i !== index && Number(ticket.ticket_id) === t.id
                              );
                              return (
                                <SelectItem
                                  key={t.id}
                                  value={String(t.id)}
                                  disabled={t.available_qty === 0 || isSelectedElsewhere}
                                >
                                  {t.ticket_type} - Rp
                    ...
                    >
                      ...
                      <SelectContent>
                        {points.length > 0 ? (
                          points.map((p) => (
                            <SelectItem key={p.id} value={String(p.id)}>
                              {new Intl.NumberFormat("id-ID").format(p.point_balance)}{" "}
                              points
                            </SelectItem>
                          ))
                        ) : (
